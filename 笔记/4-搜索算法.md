...
bool traceback(state){
    ...
    for (each candiate){
        ...
        update();
        traceback(state);
        rollback(state,candiate);
        
    }
}
```
## 搜索
> 求解最少的步数到达目标状态，或找到所有可达的状态
>
> 必要条件：状态数量在可接受的范围内 
## 回溯法
> 求解可行的某一方案或者所有方案，特征：状态数量非常大
> 
> 在转移过程中，提前减去不可能的状态
是否可以使用：
1. n的范围，一般小于等于100
2. 求唯一解：如果是判断解是否存在，通常很容易超时
3. 条件的复杂度，越复杂越容易剪枝，n的范围也可以适当放大
4. 非常取决于出题人的水平
5. 是否有其他解法
## 回溯的写法
可以拆解成5个函数：
1. traceback回溯主函数
2. can是否可以达到下一个状态或者illgeality是否非法（剪枝）
3. update更新状态
4. rollback回滚状态
5. check检测最终状态是否合法
## 回溯的技巧和延伸
...
4. Dancelinks舞蹈链
## 和贪心的对比
相同：从初始状态，通过状态转移到目标状态
不同：
贪心-每次状况转移时，可以选择最优方案，不需要枚举其他方案
回溯-无法知道状态是否是最优的，只能参与逐个枚举
