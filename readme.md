...
* ![image-20260406182336375](D:\Codes\PV-Code\assets\image-20260406182336375.png)
...
10进制转换为k进制：
1. 转换成 A-Z (如 Excel 题)当余数范围是 $0 \sim 25$ 时：
```cpp
C++res += (char)('A' + remainder); 
```
3. 转换成 16 进制 (0-9, A-F)16 进制比较特殊，因为它混合了数字和字母：
```cpp
if (remainder < 10) 
...
```
4. 纯数字进制转换 (如 2 进制、8 进制)通常不需要字符转换，直接操作 int 数组即可：
```cpp
