...
protected:
    ...
    string wName;
public:
    worker(string _workID,string _wName){
        workID=_workID;
        wName=_wName;
    }
};
...
class leader:public staff,public teacher{
private:
    int workNumber;
public:
    leader(string IDValue, string nameValue, string titleValue, string specialValue, string positionValue, int num)
    :worker(IDValue,nameValue),teacher(IDValue,nameValue,titleValue,specialValue),staff(IDValue,nameValue,positionValue),workNumber(num){}

    void display(){
        cout<<"I am a leader, my name is "<<wName<<", title is "<<title<<", speciality is "<<speciality
        <<", my position is "<<position<<" and there are "<<workNumber<<" worker in my department."<<endl;
    }
};
int main(){
